# This script trains the agent to avoid getting shot by penalizing it for losing health.
...
class DoomEnv:
    ...
    def __init__(self, game, width=80, height=60, frame_stack=4, max_steps=1000,
                 starting_steps=100, step_rate=0.05, allowed_actions=None):
        self.width = width
        self.height = height
        self.game = game
    ...
    def step(self, action_index):
        ...
        if not (done or self.episode_done):
            ...
            # Reward calculation based on health variable
            current_health = self.game.get_game_variable(vzd.GameVariable.HEALTH)
            health_loss = self.previous_health - current_health
            reward = -0.1 * health_loss  +0.01# Penalize health loss

            # Update health for the next step
...
if __name__ == "__main__":
    ...
    # Specify allowed actions (e.g., MOVE_LEFT, MOVE_RIGHT, MOVE_FORWARD)
    allowed_actions = [0, 1]  # MOVE_LEFT, MOVE_RIGHT, MOVE_FORWARD
    ...
    for episode in range(num_episodes):
        ...
        dqn_agent.plot_rewards(save_path='dodge.png')
    model_save_path = "dodge.pth"
