# This script trains a selector network to choose between three pretrained subtask models and applies a custom reward function.
# This script trains the agent to perform well in deathmatch, rewarding it for hits, item collection, and penalizes for health loss, getting stuck, and ammo usage.
...
class DoomEnv:
    ...
    def step(self, action_index):
        ...
        survival_bonus = 0.01
        new_kills = current_killcount - self.previous_killcount
        kill_reward = 50 * new_kills
        reward = kill_reward + survival_bonus + hits_reward + items_reward - health_penalty - ammo_penalty
...
if __name__ == "__main__":
    ...
    subtask_models = []
    model_paths = ['high_accuracy.pth', 'item_gathering.pth', 'dodge.pth']
    ...
    # Create a dummy CDQN instance for plotting rewards
    dummy_cdqn = CDQN(env, env, options, num_actions)
    ...
    for episode in range(num_episodes):
        ...
        # Add total reward to CDQN's episode_rewards for plotting
        dummy_cdqn.episode_rewards.append(total_reward)

        # Debugging: Log episode results
        print(f"Episode {episode + 1}: Total Reward: {total_reward}, Epsilon: {epsilon:.2f}")

        # Plot and save the rewards after each episode
        dummy_cdqn.plot_rewards(
            smoothing_window=10,
            save_path=f'multitask_deathmatch.png'
        )
        # Decay epsilon
        epsilon = max(epsilon_min, epsilon * epsilon_decay)
