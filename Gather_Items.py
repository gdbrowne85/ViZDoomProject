...
class DoomEnv:
    ...
    def __init__(self, game, width=80, height=60, frame_stack=4, max_steps=1000,
                 starting_steps=500, step_rate=0.05, allowed_actions=None):
        self.width = width
        self.height = height
        self.game = game
        ...
        self.previous_itemcount = 0  # Track items picked up
    ...
    def reset(self):
        ...
        # Initialize previous item count
    ...
    def step(self, action_index):
        ...
        if not (done or self.episode_done):
            ...
            # Reward calculation based on item collection
            current_itemcount = self.game.get_game_variable(vzd.GameVariable.ITEMCOUNT)
            items_collected = current_itemcount - self.previous_itemcount

            reward = 10 * items_collected  # Give a reward for collecting items
...
if __name__ == "__main__":
    ...
    height = 60
    game = vzd.DoomGame()
    level_path = ("deathmatch.cfg")  # Use an item-focused scenario like health_gathering.cfg
    ...
    game_variables = [vzd.GameVariable.ITEMCOUNT]  # Track items collected
    ...
    allowed_actions = [0, 1, 2, 4, 5]  # MOVE_LEFT, MOVE_RIGHT, MOVE_FORWARD, TURN_LEFT, TURN_RIGHT

    # Initialize the environment
    env = DoomEnv(game, width=width, height=height, max_steps=1000, starting_steps=1000, allowed_actions=allowed_actions)
    eval_env = env
    ...
    for episode in range(num_episodes):
        ...
        dqn_agent.plot_rewards(save_path='item_gathering.png')
        model_save_path = "item_gathering.pth"
