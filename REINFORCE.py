...
# function. In 'train_episode', reset the environment, select an action by using
# the probabilities given by the neural network, and then at the conclusion of
# the episode update the network once using a batch containing all the transitions
# in the episode. In 'pg_loss' implement the loss function for policy gradient
# without any additional improvements such as importance sampling or gradient
# clipping. Note that some improvements (normalization, Huber loss, etc.) have
# already been added to speed up learning. Keep in mind that policy gradient uses
# gradient ascent, but loss is minimized by auto-diff packages. Don't worry about
# matching the example too closely as multiple runs can have very different results
...
class Reinforce(AbstractSolver):
    ...
    def train_episode(self):
        ...
        In 'train_episode', reset the environment, select an action by using
        # the probabilities given by the neural network, and then at the conclusion of
        # the episode update the network once using a batch containing all the transitions
        # in the episode.
        """
        ...
        rewards = []  # Rewards collected per step
        action_probs = []  # Probabilities of chosen actions
        baselines = []  # Baseline values for each step
        episode_steps = 0
        ...
        # Update statistics
        self.statistics[Statistics.Rewards.value] = sum(rewards)  # Total episode reward
    ...
    def pg_loss(self, advantage, prob):
        ...
        """
        ################################
        loss = -(torch.mul(advantage,torch.log(prob)))
        return loss
        ################################
